...
import { ChevronDown, Sparkles, Zap, Box, BrainCircuit, Lightbulb } from 'lucide-react';
...
const ModelSelector = () => {
  ...
  const { selectedModel, handleModelChange } = useChat();
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute bottom-full left-0 mb-2 w-48 rounded-xl bg-ai-panel/95 backdrop-blur-xl border border-ai-border shadow-2xl overflow-hidden z-50"
          >
            <div className="p-1">
              {models.map((model) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-sm transition-colors ${
                    selectedModel === model.id ? 'bg-white/10 text-ai-text font-medium' : 'text-ai-text-muted hover:bg-white/5 hover:text-ai-text'
                  }`}
                >
